...
def start_server_container(server_image, detach_mode, port_mapping, server_params, container_name, docker_path, network="bridge"):
    cleanup_existing_container(container_name, docker_path)
    server_command = ["sudo", docker_path, "run", detach_mode, "--name", container_name]
    
    if network == "host":
        server_command.extend(["--network", "host"])
        print(f"Using host network for container: {container_name}")
    else:
        server_command.extend(["-p", port_mapping])
        print(f"Using bridge network with port mapping: {port_mapping}")
    
    if server_params:
        server_command.extend(server_params.split())
    ...
    time.sleep(10)
    print(f"Server container started with {detach_mode}, name: {container_name}, network: {network}")
    return True
...
def main():
    ...
    parser.add_argument('--port_mapping', type=str, default='8001:80', help="Port mapping for Docker in host:container format (e.g., 8001:80), used only with bridge network")
    parser.add_argument('--network', type=str, default='bridge', choices=['bridge', 'host'], help="Network mode for Docker (bridge or host, default: bridge)")
    ...
    parser.add_argument('--output_csv', type=str, default=None, help="Output CSV file path (defaults to containerName.csv in results_docker folder)")
    ...
    # Set URL based on network mode
    if args.network == "host":
        url = "http://localhost:80/"  # Adjust if server listens on a different port
    else:
    ...
    try:
        start_server_container(args.server_image, args.detach_mode, args.port_mapping, args.server_params, container_name, docker_path, args.network)
