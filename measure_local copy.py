...
def collect_resources(server, stop_event, num_cores, interval=0.5):
    """Collect CPU and memory usage for all server processes using pidstat."""
    cpu_usage = []
    mem_usage = []
    
    # Pattern for pgrep, adjusted for both servers
    pgrep_pattern = "nginx" if server == "nginx" else "beam.smp.*yaws"
    
    while not stop_event.is_set():
        # Refresh PIDs each cycle
        try:
            ...
            # Filter PIDs to include only nginx master and workers or yaws beam.smp, exclude unrelated (e.g., sudo)
            pids = []
            for pid in pids_raw:
                if not pid:
                    continue
                # Check if the process is server-related
        ...
        try:
            # Collect stats for all PIDs
            pid_list = ','.join(pids)
            result = subprocess.run(
                ["sudo", "pidstat", "-u", "-r", "-p", pid_list, "1", "1"],
                capture_output=True, text=True, check=True
            )
            lines = result.stdout.splitlines()
            print(f"pidstat output for PIDs {pid_list}:\n{result.stdout}")
            ...
            for line in lines:
                fields = line.split()
                if len(fields) >= 9 and (fields[-1] == "nginx" if server == "nginx" else "beam.smp" in fields[-1]) and "%CPU" not in line and "Command" not in line:
                    pid = fields[2]
                    # CPU usage (%CPU is column 7)
                    if fields[7].replace('.', '', 1).isdigit():
                        cpu = float(fields[7])
                        cpu_total += cpu
                        pid_count += 1
                        print(f"PID {pid} CPU usage: {cpu}%")
                    # Memory usage (RSS is column 10 in -r output)
                    if len(fields) >= 13 and fields[10].replace('.', '', 1).isdigit():
                        rss_kb = float(fields[10])  # RSS in kB
                        mem_mb = rss_kb / 1024
                        mem_total_mb += mem_mb
                        print(f"PID {pid} Memory usage: {mem_mb:.2f} MB (RSS: {rss_kb} kB)")

            # Average CPU across PIDs, sum memory
            cpu_usage.append(cpu_total / pid_count if pid_count > 0 else 0.0)
            mem_usage.append(mem_total_mb if mem_total_mb > 0 else 0.0)
            if pid_count == 0:
                print("No valid data for any PID in this cycle")
                cpu_usage.append(0.0)
                mem_usage.append(0.0)
        except subprocess.CalledProcessError as e:
